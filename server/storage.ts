...
export interface IStorage {
  ...
  createConversation(userId: string, title: string, persona: string): Promise<Conversation>;
  getConversations(userId: string, persona?: string): Promise<Conversation[]>;
...
export class DbStorage implements IStorage {
  ...
  async createConversation(userId: string, title: string, persona: string = 'chat'): Promise<Conversation> {
    try {
      ...
      const now = new Date();
      const query = `
        INSERT INTO conversations (id, user_id, title, persona, created_at, updated_at, share_token)
  ...
  async getConversations(userId: string, persona?: string): Promise<Conversation[]> {
    try {
      let query: string;
      let params: string[];
      
      if (persona) {
        query = `SELECT * FROM conversations WHERE user_id = $1 AND persona = $2 ORDER BY updated_at DESC`;
        params = [userId, persona];
      } else {
